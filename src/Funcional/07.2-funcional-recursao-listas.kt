...
fun main() {
    ...
    fun separarParesImpares(lista: List<Int>): Pair<List<Int>, List<Int>> {
        tailrec fun pi(list: List<Int>, lpares: List<Int> = listOf(), limpares: List<Int> = listOf()): Pair<List<Int>, List<Int>>{
            if(list.isEmpty()) return Pair(limpares, lpares)
    ...
    fun abastecimentoCasas(N: Int, caminhoes: List<Triple<Int, Int, Int>>): List<Int> {
        tailrec fun abastecer(A: Int, B: Int, L: Int, casas: List<Int>, acumulador: List<Int>, index: Int = 0): List<Int>{
            if(index == casas.size) return acumulador
            if(index !in A..B) return abastecer(A, B, L, casas, acumulador + listOf(casas[index]), index+1)
            return abastecer(A, B, L, casas, acumulador + listOf(casas[index] + L), index+1)
        }
        tailrec fun abastecerTodas(caminhoes: List<Triple<Int, Int, Int>>, casas: List<Int>): List<Int>{
            if(caminhoes.isEmpty()) return casas
            return abastecerTodas(caminhoes.drop(1), abastecer(caminhoes.first().first, caminhoes.first().second, caminhoes.first().third, casas, listOf<Int>()))
        }
