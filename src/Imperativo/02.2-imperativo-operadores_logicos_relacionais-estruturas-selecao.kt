...
fun main() {
    ...
    */
    ...
    fun formiga(hora: Int, minuto: Int, sentido: Char, distancia: Int): Pair<Int, Int> {
        var minutosDistancia = distancia*10 // ex 6 cm = 60 minutos
        var horasDistancia = minutosDistancia/60 // 60 minutos = 1 hora
        minutosDistancia = minutosDistancia % 60 // minutos que sobraram
        //println("Horas andadas: $horasDistancia, minutos andados: $minutosDistancia")

        var novaHora = hora
        var novoMinuto = minuto
        if(sentido == 'H'){
            novaHora = novaHora + horasDistancia
            novoMinuto += minutosDistancia
            //println("Nova hora e novo minuto sem maracutaia de tempo: $novaHora, $novoMinuto")
            if(novoMinuto >= 60){
                novaHora = novaHora + 1
                novoMinuto = novoMinuto % 60
            }
            novaHora = novaHora % 12
            //println("Nova hora e novo minuto com maracutaia de tempo: $novaHora, $novoMinuto")

        }else{
            novaHora = (novaHora - horasDistancia)
            novoMinuto = (novoMinuto - minutosDistancia)
            //println("Nova hora e novo minuto sem maracutaia de tempo: $novaHora, $novoMinuto")
            if(novoMinuto < 0){
                novaHora = (novaHora - 1)
                novoMinuto += 60
            }
            if(novaHora < 0) novaHora = novaHora % 12 + 12
            //println("Nova hora e novo minuto com maracutaia de tempo: $novaHora, $novoMinuto")
        }
            return Pair(novaHora, novoMinuto)
    }
